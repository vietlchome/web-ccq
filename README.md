...
- **Tín hiệu mua**: mỗi `X` phiên (chu kỳ DCA), tùy chọn chỉ mua khi NAV < EMA200. Tín hiệu dựa trên NAV **phiên trước** (`close[1]`, `ema[1]`, `sd[1]`), khớp lệnh tại NAV **phiên nay** — giữ nguyên tinh thần fix lookahead của bản Pine ("hôm qua đóng nến thấy tín hiệu → hôm nay đặt lệnh", phù hợp cơ chế khớp NAV kỳ tới của quỹ mở).
- **Hệ số nhân động**: theo %EMA (NAV dưới EMA200 mỗi X% → +1x); chỉ >1x khi NAV dưới EMA200; làm tròn 1 chữ số thập phân, sàn 1.0x.
